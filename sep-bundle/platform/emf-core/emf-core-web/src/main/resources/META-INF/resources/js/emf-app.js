...
EMF.dom = {
	...
	buildCheckTable: function(options) {
		...
		for ( var item in config.data) {
			...
			html += '<tr>';
			html += '<td class="check-column"><input type="' + selectType + '" value="' + current.instanceId + '" name="' + selectorName + '" /></td>';
...
EMF.search.quick = {
	...
	init: function() {
		 ...
		 */
		$(EMF.search.quick.SEARCH_RIGHT_MENU).keypress(function(event) {
		...
		$('<div class="loading-mask">&#160;</div>').insertAfter(element)
        	.hide()
           	.css({
                ...
                'background-image' : 'url("' + SF.config.contextPath + '/images/ajax-loader.gif")',
...
EMF.notify = {
	...
	createHtml : function(data) {
		var popup =
			'<div class="notifier ' + data.level + '" title=" "> \
				<div class="notifier-content">' + data.message.html() + '</div> \
			</div>';
		return popup;
	},

	openPopup : function(popup, config) {
		var dialogClass = 'notifyme custom ' + config.level;
		var popupConfig = {
			resizable 		: false,
			height		 	: 'auto',
			width			: 'auto',
...
EMF.dialog = {
    ...
    config: {
        ...
        width       : 300,// consider width to be 'auto'
        height      : 150,// consider height to be 'auto'
        resizable   : false,
        // Default dialog css class.
        dialogClass : 'notifyme custom ',
