...
public class SecurityContextManager {
	...
	public void init() {
		...
		UserInfo info = new UserInfo(systemUserDisplayName, null);
		info.put(ResourceProperties.LANGUAGE, systemLanguage);
		EmfUser emfUser = (EmfUser) SYSTEM_USER;
		emfUser.setId("emf:" + emfUser.getIdentifier());
		emfUser.setUserInfo(info);
		emfUser.setTenantId(defaultContainer);
		USE_TIME_CONSTRAINTS = useTimeConstraints;
	}
	...
	public static String getUserLanguage(Resource resource) {
		...
		if (user.getUserInfo().getLanguage() == null) {
			return systemLanguage;
		}
	...
	public synchronized static User getAdminUser() {
		if (initializedAdmin) {
			...
			UserInfo info = new UserInfo(adminUser, null);
			info.put(ResourceProperties.LANGUAGE, systemLanguage);
			emfUser.setUserInfo(info);
