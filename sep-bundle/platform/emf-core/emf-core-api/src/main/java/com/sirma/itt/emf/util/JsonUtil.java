...
public class JsonUtil {
	...
	public static void addToJson(JSONObject object, String key, Object value) {
		...
		try {
			...
			LOGGER.warn("Failed to put to entry with key " + key + " and value " + value, e);
			try {
				object.put(key, (String) null);
			} catch (JSONException e1) {
				LOGGER.warn("Failed to put to entry with key " + key + " and value null", e1);
			}
		}
	}
	...
	public static JSONArray createArrayFromString(String arrayString) {
		try {
			...
			LOGGER.warn("Failed to parse string  ", e);
		}
		return null;
	}
