 ...
 * provide cache configurations and implement the abstract persistence methods.
 *
 * @author BBonev
 */
public abstract class AbstractCommentService implements CommentService, Serializable {
	...
	public void initialize() {
		FORBIDDEN_PROPERTIES = new LinkedHashSet<String>(50);
...
//			EntityLookupCache<Serializable, Serializable, Serializable> topicIdByCommentCache = getTopicIdByCommentCache();
...
//			}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
	public List<TopicInstance> getTopics(InstanceReference reference, Date lastKnownDate, int limit, boolean includeComments, Sorter sort, Map<String, Serializable> filterParams) {
		if ((reference == null) || (reference.getIdentifier() == null)) {
			return Collections.emptyList();
		}
		return getTopicsByTargetKey(reference, lastKnownDate, limit, includeComments, sort, filterParams);
	}
	...
	private List<TopicInstance> getTopicsByTargetKey(Serializable reference, Date lastKnownDate,
			int limit, boolean includeComments, Sorter sorter, Map<String, Serializable> filterParams) {
		// EntityLookupCache<Serializable, Set<Serializable>, Serializable> cache =
		// getTopicIdByTargetCache();
		// Set<Serializable> set = cache.getValue(reference);
		...
		// } else {
		// EntityLookupCache<Serializable, TopicInstance, Serializable> idCache =
		// getTopicByIdCache();
		// topics = new ArrayList<>(set.size());
		// for (Serializable id : set) {
		// Pair<Serializable, TopicInstance> topicPair = idCache.getByKey(id);
		// if ((topicPair != null) && (topicPair.getSecond() != null)) {
		// // NOTE: this will result the direct reference to the cache entry
		// topics.add(createCopy(topicPair.getSecond()));
		// }
		// }
		// }
	 ...
	 */
	@Override
	public List<TopicInstance> getTopics(String subSectionIdentifier, Date lastKnownDate, int limit, boolean includeComments, Sorter sort, Map<String, Serializable> filterParams) {
	...
	public CommentInstance save(CommentInstance instance, Operation operation) {
		...
		if (instance instanceof TopicInstance) {
			...
			if (isNew) {
				...
				Pair<Serializable, TopicInstance> byKey = getTopicByIdCache().getByKey(topic.getId());
				if (byKey != null) {
					old = byKey.getSecond();
				} else {
		...
		if ((comentToTopic != null) && (comentToTopic.getSecond() != null)) {
			...
			if ((pair != null) && (pair.getSecond() != null)) {
				...
				if (comment != null) {
					...
					eventService.fire(new CommentUpdatedEvent(null, comment));
	...
	public List<CommentInstance> postComment(Serializable topicId, Date lastCommentDate,
		...
		if (pair != null) {
			...
			if (loadComments.isEmpty()) {
				loadComments = new LinkedList<CommentInstance>();
			}
	...
	/**
	 * Adds the topic to target cache.
	 *
	 * @param key
	 *            the key to the cache
	 * @param topicId
	 *            the topic id to add to the cache
	 * @param topicAboutCache
	 *            the cache to update
	 */
	private void addTopicToTargetCache(Serializable key, Serializable topicId,
			EntityLookupCache<Serializable, Set<Serializable>, Serializable> topicAboutCache) {
		if ((key == null) || (topicId == null)) {
			return;
		}
		Set<Serializable> set = topicAboutCache.getValue(key);
		if (set == null) {
			set = new LinkedHashSet<>();
		}
		set.add(topicId);
		// update the value back in the cache
		topicAboutCache.setValue(key, set);
	}
	...
	public class TopicIdByTargetLookup extends
		...
		public Pair<Serializable, Set<Serializable>> findByKey(Serializable key) {
			...
			// topics = loadTopicByTopicAbout((InstanceReference) key);
			// } else if (key instanceof String) {
			// topics = loadTopicBySubSectionId((String) key);
			// }
			if ((topics == null) || topics.isEmpty()) {
				return null;
			}
			Set<Serializable> ids = new LinkedHashSet<>(topics.size());
			EntityLookupCache<Serializable, TopicInstance, Serializable> cache = getTopicByIdCache();
			for (TopicInstance instance : topics) {
				ids.add(instance.getId());
				initializeCommentsToTopicCache(instance);
				cache.setValue(instance.getId(), instance);
			}
