...
/**
 * Utility class used for exporting specific object from emf to pdf. The class
 * uses EMF bookmarkable URLs and the tool wkhtmltopdf (<a
 * href="http://wkhtmltopdf.org/">http://wkhtmltopdf.org/</a>).
 * 
 * @author Ivo Rusev
 */
public class PDFExporter {
	...
	/**
	 * The thread execution pool, used to store the wkhtmltopdf processes. Used
	 * to terminate the processes that take too long.
	 */
	...
	/**
	 * Export timeout. After this time the execution of the wkhtmltopdf will be
	 * interrupted.
	 */
	...
	/**
	 * The path to the OS specific (has to escape right symbols for each OS) js
	 * that is executed after wk has finished.
	 */
	@Inject
	...
	public String exportToPdf(final String bookmarkableUrl) {
		...
		try {
			...
			try {
				...
				}, EXPORT_TIMEOUT_IN_SECONDS, TimeUnit.SECONDS);
