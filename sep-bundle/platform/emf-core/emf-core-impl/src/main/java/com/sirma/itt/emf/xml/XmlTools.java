...
public class XmlTools {
	 ...
	 */
	private static Pattern MATCH_EMPTY_LINES = Pattern
			.compile("\r?\n\\s*\r?\n");
	...
	public static String stripEmptyTags(String xml) {
		...
		try {
			...
			// Syntax error in the replacement text (unescaped $
			// signs?)
		} catch (IndexOutOfBoundsException ex) {
			// Non-existent back reference used the replacement text
		}
		return xml;
	}
	...
	public static String addSchemaDeclaration(String xml, String schemaName) {
		...
		if ((declaredSchemaName != null)
				&& !schemaName.equalsIgnoreCase(declaredSchemaName)) {
			LOGGER.warn("Found schema declaration " + declaredSchemaName
					+ " that does not match expected schema " + schemaName);
		}
		String schemaLocation = SCHEMA_LOCATION_FORMATTER.format(new String[] {
				schemaName });
		...
		xmlBuilder.append('>').append(matcher.group(6)).append("</").append(
				xmlRootElement).append('>');
