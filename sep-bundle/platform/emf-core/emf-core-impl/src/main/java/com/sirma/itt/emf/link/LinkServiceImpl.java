...
public class LinkServiceImpl extends AbstractLinkService {
	...
	private int unlinkInternal(InstanceReference fromId, InstanceReference toId, String linkId) {
		...
		if (fromId.equals(toId)) {
			...
			loadedLinks.addAll(loadedLinks);
		...
		// // delete links taking in account for start and end but only not the id
		// String query = EmfQueries.DELETE_LINKS_FOR_INSTANCE_KEY;
		// List<Pair<String, Object>> args = new ArrayList<Pair<String, Object>>(5);
		// if (linkId != null) {
		// if (fromId.equals(toId)) {
		// // delete links without taking in account for start and end but only the id
		// query = EmfQueries.DELETE_LINK_FOR_INSTANCE_AND_TYPE_KEY;
		// } else {
		// // delete links taking in account for start, end and id
		// query = EmfQueries.DELETE_LINK_FOR_INSTANCE_KEY;
		// }
		// args.add(new Pair<String, Object>("linkId", linkId));
		// } else if (fromId.equals(toId)) {
		// args.clear();
		// // delete links without taking in account for start, end and id
		// query = EmfQueries.DELETE_ALL_LINKS_FOR_INSTANCE_KEY;
		// }
		//
		// args.add(new Pair<String, Object>("fromId", fromId.getIdentifier()));
		// args.add(new Pair<String, Object>("fromType", fromId.getReferenceType().getId()));
		// args.add(new Pair<String, Object>("toId", toId.getIdentifier()));
		// args.add(new Pair<String, Object>("toType", toId.getReferenceType().getId()));
