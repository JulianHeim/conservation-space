...
public class SchemaBuilder {
	...
	private final static Map<String, Schema> cachedSchemas = new LinkedHashMap<String, Schema>();
	/**
	 * <p>
	...
	public static Schema getSchema(Class<?> baseLoaderClass, String mainSchemaPath,
		...
		if ((additional != null) && (additional.length > 0)) {
			...
			try {
				...
				files[0] = loadString(baseLoaderClass.getResourceAsStream(mainSchemaPath));
				for (int i = 0; i < additional.length; i++) {
					...
					if (StringUtils.isNotNullOrEmpty(uri)) {
						...
						files[i + 1] = loadString(inputStream);
					}
				}
				// merge files to a single XSD file
				String mergeXSD = XmlTools.mergeXSD(files);
				schemas[0] = new StreamSource(new StringReader(mergeXSD));
			} catch (IOException e) {
	...
	/**
	 * Reads the given input stream and loads it to String
	 * 
	 * @param stream
	 *            is the stream to load
	 * @return is the stream content
	 * @throws IOException
	 *             if error occur during stream manipulation
	 */
