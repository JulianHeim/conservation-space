 ...
 * The error handler can be tuned to collect the errors or to throw an exception
 * on error. The default behavior is to collect the errors, if you need the
 * other case you need to call the method
...
public class ErrorHandler extends DefaultHandler {
	...
	private List<XmlError> errorsList = new LinkedList<XmlError>();
	...
	/**
	 * Returns <code>true</code> if after parsing there are errors and
	 * <code>false</code> if no errors detected.
	 * 
	 ...
	 *            is the error level.
	 */
	private void addError(SAXParseException e, XmlErrorLevel level) {
		errorsList.add(new XmlError(e.getMessage(), level, e.getLineNumber(), e
				.getColumnNumber()));
	}
