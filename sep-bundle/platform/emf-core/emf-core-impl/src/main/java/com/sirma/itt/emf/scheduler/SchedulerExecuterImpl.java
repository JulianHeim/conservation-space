...
public class SchedulerExecuterImpl implements SchedulerExecuter {
	...
	private static final Logger LOGGER = Logger.getLogger(SchedulerExecuterImpl.class);
	/** The dao. */
	@Inject
	private DbDao dao;
	...
	public void execute(SchedulerEntry schedulerEntry) {
		...
		try {
			...
			LOGGER.error("Failed to execute action in " + (sameTransaction ? "same" : "new")
					+ " transaction due to: " + e.getMessage(), e);
