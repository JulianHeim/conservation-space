...
/**
 * {@link TopicInstance} and {@link CommentInstance} <-> {@link JSONObject}
 * converter.
 * 
 * @author yasko
 */
...
public class CommentConverterProvider implements TypeConverterProvider {
	...
	public class JsonObjectToTopicInstanceConverter implements Converter<JSONObject, TopicInstance> {
		...
		public TopicInstance convert(JSONObject source) {
			...
			if (about != null) {
				String aboutId = JsonUtil.getStringValue(about, "instanceId");
				String aboutType = JsonUtil.getStringValue(about, "instanceType");
			...
			if (shapeJson != null) {
				...
				shape.setSvgValue(JsonUtil.getStringValue(shapeJson, "svgTag"));
				shape.setViewBox(JsonUtil.getStringValue(shapeJson, "viewbox"));
				String zoomLevel = JsonUtil.getStringValue(shapeJson, "zoomLevel");
	...
	public class CommentInstanceToJsonConverter implements Converter<CommentInstance, JSONObject> {
		...
		private JSONObject convertTopic(TopicInstance instance) {
			...
			JsonUtil.addToJson(about, "instanceId", instance.getTopicAbout().getIdentifier());
			JsonUtil.addToJson(about, "instanceType", instance.getTopicAbout().getReferenceType()
					.getName().toLowerCase());
