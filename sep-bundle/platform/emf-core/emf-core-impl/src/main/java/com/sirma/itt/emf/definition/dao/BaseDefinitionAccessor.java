...
public abstract class BaseDefinitionAccessor implements Serializable {
	...
	/** The Constant LOGGER. */
	protected static final Logger LOGGER = Logger.getLogger(BaseDefinitionAccessor.class);
	...
	@Inject
	protected Instance<MutableDictionaryService> dictionaryServiceInstance;
	...
	protected <E extends DefinitionModel> List<E> getDefinitionsInternal(String query,
		...
		if (debug) {
			...
			LOGGER.debug("Db search for " + typeName + " definitions took "
		...
		if (convert) {
			...
			if (debug) {
				LOGGER.debug("Definitions filterring took " + tracker.stopInSeconds() + " s");
			}
			if (updateCache) {
			...
			if (debug) {
				String typeName = type != null ? type.getTitle() : "all";
				LOGGER.debug("Fetched " + filteredDefinitions.size() + " " + typeName
	...
	protected <E extends DefinitionModel> void updateCache(List<E> definitions,
		...
		if (debug) {
			...
			LOGGER.debug("Cache refresh for " + definitions.size() + " " + defName
					+ "s definitions completed in " + tracker.stopInSeconds() + " s");
		}
	}
	 ...
	 */
	protected MutableDictionaryService getDictionaryService() {
