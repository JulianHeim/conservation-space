...
import com.sirma.itt.idoc.web.document.actions.IdocAction;
import com.sirma.itt.idoc.web.document.actions.IdocActionDefinition;
...
public class IntelligentDocumentRestService {
	...
	public String saveInstanceProperties(@PathParam("id") String instanceId,
		...
		Map<String, Serializable> fromRest = propertiesService.convertFromRest(properties,
				instanceDefinition);
		...
		// propertiesService.saveProperties(instance, false);
		//
	...
	public String getiDocSections(@QueryParam("caseId") String caseId) {
		...
		try {
			...
			LOGGER.error("Can not convert given instance to json object", e);
		}
		...
		if (SequenceEntityGenerator.isPersisted(rest) && (rest.getType() != null)) {
			...
			if (instance != null) {
				...
				instance.getProperties().putAll(
						propertiesService.convertFromRest(rest.getProperties(), definitionModel));
				instance.getProperties()
						.putAll(propertiesService.convertFromRest(rest.getViewProperties(),
								definitionModel));
	...
	public Object initializeNewInstance(@QueryParam("type") String type,
		...
		Instance instance = instanceService
				.createInstance(definitionModel, null, new Operation(""));
	...
	public RestInstance create(@Context HttpServletRequest request, RestInstance rest) {
		...
		if ((rest.getOwningInstanceId() != null) && (rest.getOwningInstanceType() != null)) {
		...
		if ((owningInstance instanceof SectionInstance) && (typeClass != null)
			...
			if (newDocumentInstance == null) {
				// the document definition is probably not allowed in the section we should create
				// from template
				...
				// TODO: operation should be added if the document should be initialized in a state
			...
			// StringEscapeUtils.escapeHtml(str) break cyrillic titles - CMF-6044
			properties.put(DocumentProperties.TITLE,
	...
	public String getVersions(@PathParam("id") String id, @QueryParam("type") String type) {
		...
		try {
			...
			// versions are returned in latest-first order that's way we iterate in reverse order
	 ...
	 * @param type
	 *            the type
	 * @return JSON object containing a single property ('lockedBy') if this
	 *         property has a value it means that a document is already locked,
	 *         otherwise it was successfully locked.
	 */
	@POST
	@Path("/{id}/edit")
	...
	public void deleteDocument(@QueryParam("documentId") String documentId,
		...
		if (!view.isLocked() || isLockedByMe(view)) {
			instanceService.delete(instance, new Operation(""), true);
		}
	}
	...
	private Map<String, Object> convertProperties(Instance instance) {
		DefinitionModel definitionModel = dictionaryService.getInstanceDefinition(instance);
		return (Map<String, Object>) propertiesService.convertForRest(instance, definitionModel);
	}
	...
	@GET
	@Path("/actions")
	public List<Action> getAllowedActions(@QueryParam("id") String id,
			@QueryParam("type") String type) {
		// REVIEW action fetching should be revised according to the permission
		// model

		// FIXME: move to convertToresponce method and use EMF actions mechanism
		Set<Action> actions = new LinkedHashSet<>();
		actions.add(new IdocAction(IdocActionDefinition.DOCUMENT_CREATE, labelProvider
				.getValue("idoc.btn." + IdocActionDefinition.DOCUMENT_CREATE.getId())));
		actions.add(new IdocAction(IdocActionDefinition.DOCUMENT_DELETE, labelProvider
				.getValue("idoc.btn." + IdocActionDefinition.DOCUMENT_DELETE.getId() + "." + type)));
		actions.add(new IdocAction(IdocActionDefinition.DOCUMENT_CLONE, labelProvider
				.getValue("idoc.btn." + IdocActionDefinition.DOCUMENT_CLONE.getId() + "." + type)));
		actions.add(new IdocAction(IdocActionDefinition.SAVE_AS_TEMPLATE, labelProvider
				.getValue("idoc.btn." + IdocActionDefinition.SAVE_AS_TEMPLATE.getId())));
		actions.add(new IdocAction(IdocActionDefinition.DOCUMENT_UPLOAD, labelProvider
				.getValue("idoc.btn." + IdocActionDefinition.DOCUMENT_UPLOAD.getId() + "." + type)));
		actions.add(new IdocAction(IdocActionDefinition.DOCUMENT_PRINT, labelProvider
				.getValue("idoc.btn." + IdocActionDefinition.DOCUMENT_PRINT.getId())));
		actions.add(new IdocAction(IdocActionDefinition.DOCUMENT_EXPORT, labelProvider
				.getValue("idoc.btn." + IdocActionDefinition.DOCUMENT_EXPORT.getId() + "." + type)));
	...
	public RestInstance clone(@Context HttpServletRequest request,
		...
		RestInstance restInstance = convertInstanceToResponce(instance);
		Instance clonedInstance = serviceRegister.getInstanceService(instance.getClass()).clone(
				instance, new Operation("clone"));
		...
		restClonedInstance.setCurrentOperation("clone");
