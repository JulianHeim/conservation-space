...
public class SemanticQueryVisitor extends AbstractQueryVistor {
	...
	public void start() {
		...
		whereClause.append(QUERY_OBJECT_VARIABLE).append(" sesame:directType ")
				.append(QUERY_OBJECT_TYPE_VARIABLE).append(" . ")
				.append(QUERY_OBJECT_VARIABLE)
	...
	public void visit(Query query) throws Exception {
		...
		if ("context".equals(predicate)) {
			...
			filterClause.append(QUERY_BLOCK_START);
			String filterClauseRelation = buildListFilter(subject, predicate, (List<String>) value);
			filterClause.append(filterClauseRelation);
			filterClause.append("?relation a emf:Relation .");
			...
			for (StringBuilder builder : filterClauseList) {
				if (builder.indexOf("?relationVariable") > 0) {
			...
			if (filterClause.length() == 0) {
				filterClause.append(QUERY_BLOCK_START).append(QUERY_OBJECT_VARIABLE).append(" ")
						.append("?relation").append(" ").append("?context")
						.append(QUERY_STATEMENT_SEPARATOR);
			...
			int relationIndexOf = filterClause.indexOf("emf:Relation");

			filterClause.insert(relationIndexOf + "emf:Relation".length() + 3, relationTypeFilter);
			...
			if ("fts".equals(predicate)) {
				...
				if (fullTextsearchIndex.startsWith("luc")) {
					...
					filterClause.append("?search a ").append(fullTextsearchIndex)
							.append(" ; solr:query ").append(QUERY_VARIABLE).append(valueVariable)
							.append(" ; solr:entities ").append(QUERY_OBJECT_VARIABLE)
							.append(QUERY_STATEMENT_SEPARATOR);
				}
	 ...
	 *
	 * @param maxResultLimit the maxResultLimit to set
	 */
