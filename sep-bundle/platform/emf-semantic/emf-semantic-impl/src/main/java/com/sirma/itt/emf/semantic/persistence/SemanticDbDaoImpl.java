	...
	protected Instance saveOrUpdateInternal(Instance entity, Instance oldEntity) {
		...
		try {
			...
			if (oldEntity != null) {
				...
				// add the short URI as a data property
				// of the subject to be synchronised with Solr
				// TODO : to be removed on subsequent
				//        Ontotext Forest Solr Connector release
				addModel.add(subject, EMF.URI,
						valueFactory.createLiteral(entity.getId().toString()),
						getContext());
				// ensure the isDeleted is set to false
				addModel.add(subject, EMF.IS_DELETED, valueFactory.createLiteral(false),
						getContext());
			}

			// TODO remove this when we start working with Lucene4
			URI luceneIndex = valueFactory.createURI(Lucene.NAMESPACE, "ftsearch");
			addModel.add(luceneIndex, Lucene.ADD_TO_INDEX, subject, getContext());
