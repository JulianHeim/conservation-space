...
public class SemanticPropertiesWriteConverter extends BasePropertiesConverter {
	 ...
	 */
	private Object buildStatements(PropertyDefinition field, Serializable value, Serializable oldValue,
			Resource subject, Set<String> persistedProperties, Model addModel, Model removeModel) {
		...
		if (value != null) {
			...
			if (isResourceField(field)) {
				...
				if (value instanceof Instance) {
					...
					debug(LOGGER, "Passed a transient instance to persist for key=", field.getName(),
							" and uri=", field.getUri());
				} else if (value instanceof Entity) {
					return createStatementForUriValue(subject, ((Entity<?>) value).getId(),
							field.getUri());
	...
	private Serializable getUserUri(Serializable value) {
		if (value instanceof Instance) {
			...
			if (((String) value).startsWith("{")) {
				InstanceReference instance = typeConverter.convert(InstanceReference.class, value);
				if (instance != null) {
					return instance.getIdentifier();
				}
			} else // for single value
			if (value.toString().contains(NamespaceRegistryService.SHORT_URI_DELIMITER)) {
				return value;
			}
		}
		// will try to load it as regular user
		com.sirma.itt.emf.resources.model.Resource resource = resourceService.getResource(
				value.toString(), ResourceType.USER);
		if (resource != null) {
