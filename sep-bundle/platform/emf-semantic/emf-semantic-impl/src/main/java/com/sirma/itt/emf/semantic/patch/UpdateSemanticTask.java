...
import com.sirma.itt.emf.semantic.ConnectionFactoryUtils;
...
public class UpdateSemanticTask implements CustomTaskChange {
	...
	public void setUp() throws SetupException {
		connectionFactory = ConnectionFactoryUtils.getConnectionFactory();
		...
		try {
			...
			for (String fileName : importFiles) {
				...
				if (FILE_TYPE_RDF.equals(fileType)) {
					RepositoryCreatorUtils.addFileToRepository(repositoryConnection, fileName);
				} else if (FILE_TYPE_SPARQL.equals(fileType)) {
					// execute sparql update file
					InputStream file = null;
					try {

						file = resourceAccessor.getResourceAsStream(fileName);
						if (file != null) {
