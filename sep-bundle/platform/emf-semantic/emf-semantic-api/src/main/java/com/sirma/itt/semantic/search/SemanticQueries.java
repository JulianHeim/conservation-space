...
public enum SemanticQueries {
	...
	QUERY_CLASSES_TYPES_FOR_SEARCH(
			"QUERY_CLASSES_TYPES_FOR_SEARCH",
			"SELECT distinct ?instance (owl:Class as ?instanceType) ?title ?definition ?superClass ((if(!bound(?isSearchable), \"false\"^^xsd:boolean , ?isSearchable)) as ?searchable) where {\n"
					+ "{\n"
	...
	QUERY_RELATION_PROPERTIES(
			"QUERY_RELATION_PROPERTIES",
			"SELECT distinct ?instance (emf:Relation as ?instanceType) ?title ?definition ((if (bound(?domainClass1), ?domainClass1, ptop:Entity)) as ?domainClass) ?rangeClass where {\n"
	...
	QUERY_GET_TOPICS_BY_OBJECT_ID(
			"QUERY_GET_TOPICS_BY_OBJECT_ID",
			"SELECT distinct ?instance ?instanceType ?type ?status ?object ?objectType ?objectSection ?title ?content ?createdOn ?modifiedOn ?createdBy ?tags ?imageAnnotation ?svgValue ?zoomLevel ?viewBox  WHERE { " //(count( distinct ?reply) as ?comments)
					...
					+ " {category} "
					+ "optional {?instance emf:content ?content .} " // this should not be optional but for the old comments to work it is - until db fix is implemented
					+ " {objectTypeParameter} "
					+ "?object sesame:directType ?objectType . "
					+ " {imageAnnotation} "
					+ "?instance sesame:directType ?instanceType . "// this one could be omitted??
...
//					+ "group by ?instance ?instanceType ?type ?status ?object ?objectType ?objectSection ?title ?content ?createdOn ?createdBy ?tags ?parent ?imageAnnotation ?svgValue ?zoomLevel ?viewBox "
					+ "", Arrays.asList(new StringPair("datesFilter", ""),
							...
							new StringPair("imageAnnotation", "optional { ?instance emf:hasImageAnnotation ?imageAnnotation . ?imageAnnotation emf:svgValue ?svgValue ; emf:zoomLevel ?zoomLevel ; emf:viewBox ?viewBox. }"),
...
//					+ "filter ( NOT EXISTS { ?object emf:isDeleted \"true\"^^xsd:boolean } ) . "
	...
	QUERY_CLASSES_PART_OF_OBJECT_LIBRARY(NamedQueries.QUERY_CLASSES_PART_OF_OBJECT_LIBRARY,
			"select ?instance (owl:Class as ?instanceType) ?title where {"
					+ "?instance a rdfs:Class ."
					+ "?instance emf:isPartOfObjectLibrary \"true\"^^xsd:boolean ."
					+ "?instance rdfs:label ?title ."
					+ "FILTER(langMatches(lang(?title), if(bound(?lang), ?lang, \"en\")))"
					+ "} order by ?title", null);
