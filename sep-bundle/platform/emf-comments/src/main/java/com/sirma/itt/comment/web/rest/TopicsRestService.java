...
public class TopicsRestService extends CommonCommentRestService {
	...
	public Response findAll(@QueryParam("sectionId") String sectionId,
			...
			@QueryParam("includeChildren") Boolean includeSubInstances,
			@QueryParam("user") String user, @QueryParam("sortBy") String sortByParam,
			@QueryParam("sortDirection") String sortDirectionParam,
		...
		String sortBy = StringUtils.isBlank(sortByParam) ? DefaultProperties.CREATED_ON
				: sortByParam;
		String sortDirection = StringUtils.isBlank(sortDirectionParam) ? Sorter.SORT_DESCENDING
				: sortDirectionParam.toLowerCase();
		Sorter sorter = new Sorter(sortBy, sortDirection);
		...
		int max = limit == null ? 25 : limit.intValue();
		...
		if (StringUtils.isNotBlank(sectionId)) {
			...
			if (Boolean.TRUE.equals(includeSubInstances)) {
				...
				return buildBadRequest("Invalid arguments configuration");
			}
	...
	private Map<String, Serializable> createFilterMap(String dateFrom, String dateTo,
			String userFilter, String tags, String category, String status, String level, String keyword, String type) {
