...
public class ObjectRoleEvaluator extends BaseRoleEvaluator<ObjectInstance> {
	...
	private static final Action EDIT_DETAILS = new EmfAction(ActionTypeConstants.EDIT_DETAILS);
	/** The Constant DELETE. */
	private static final Action DELETE = new EmfAction(ActionTypeConstants.DELETE);
	private static final Action MOVE_TO_OTHER_SECTION = new EmfAction(
			ObjectActionTypeConstants.OBJECT_MOVE_SAME_CASE);
	private static final Action LOCK = new EmfAction(ActionTypeConstants.LOCK);
	private static final Action UNLOCK = new EmfAction(ActionTypeConstants.UNLOCK);
	private static final Action CLONE = new EmfAction(ActionTypeConstants.CLONE);
	private static final Action PRINT = new EmfAction(ActionTypeConstants.PRINT);
	private static final Action EXPORT = new EmfAction(ActionTypeConstants.EXPORT);
	...
	private static final Set<Action> INACTIVE_TARGET_ALLOWED_ACTIONS = new HashSet<Action>(
			Arrays.asList(CLONE, PRINT, EXPORT));
	...
	protected Pair<Role, RoleEvaluator<ObjectInstance>> evaluateInternal(ObjectInstance target,
		...
		if (resourceService.equals(user, properties.get(DefaultProperties.CREATED_BY))) {
	...
	protected Boolean filterInternal(ObjectInstance target, Resource resource, Role role,
		...
		if ((viewInstance != null) && viewInstance.isLocked()) {
			...
			// locked from before
			if (!resourceService.equals(viewInstance.getLockedBy(), resource)) {
