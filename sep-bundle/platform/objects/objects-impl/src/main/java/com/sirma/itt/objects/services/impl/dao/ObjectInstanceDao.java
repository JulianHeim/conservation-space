...
public class ObjectInstanceDao extends
	...
	protected <E extends Entity<Serializable>> List<E> findEntities(Set<String> ids) {
		...
		// DbQueryTemplatesObjects.QUERY_OBJECT_ENTITIES_BY_DMS_ID_KEY,
	...
	protected <E extends Entity<Serializable>> List<E> findEntitiesByPrimaryKey(
		...
		// ObjectInstance instance = getDbDao().find(getInstanceClass(), serializable);
		// if (instance != null) {
		// result.add((E) instance);
		// }
		// }
		// return result;
		List<E> list = getDbDao().fetchWithNamed("SELECT_BY_IDS",
		...
		if (view != null) {
			try {
				RuntimeConfiguration.setConfiguration(
						RuntimeConfigurationProperties.DISABLE_AUTOMATIC_LINKS, Boolean.TRUE);
				view.getProperties().put(DefaultProperties.TRANSIENT_SEMANTIC_INSTANCE, Boolean.TRUE);
				documentService.save(view, OPERATION);
			} finally {
				RuntimeConfiguration
						.clearConfiguration(RuntimeConfigurationProperties.DISABLE_AUTOMATIC_LINKS);
