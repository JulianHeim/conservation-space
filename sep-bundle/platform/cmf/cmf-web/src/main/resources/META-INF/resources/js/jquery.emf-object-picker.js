	...
	$.fn.objectPicker = function(options) {
	    ...
	    var defaults = {
        	...
        	dialogConfig	: {
    	        ...
    	        dialogClass : "object-picker-dialog notifyme",
    	        modal : true,
    	        width : 1000,
    	        height : 620,
    	        resizable : true
		...
		plugin.panel = {
			...
			'browse': function(options) {
	    		...
	    		panel.on('object.tree.selection', function(evt) {
	    			if(evt.selectedNodes.length > 0) {
	    				if(plugin.settings.browserSelectDeselectCallback) {
		...
		return this.each(function(index) { 
			var $this = $(this);
			var diloagSettings = plugin.settings.dialogConfig;
		    ...
		    if (plugin.settings.popup) {
		    	...
		    	var dialogConfiguration = {
    	    		...
    	    		modal        : diloagSettings.modal,
    	    		width        : diloagSettings.width,
    	    		height       : diloagSettings.height,
    	    		resizable    : diloagSettings.resizable,
    	    		dialogClass  : diloagSettings.dialogClass,
    	    ...
    	    var html = 
	    	'<div class="object-picker full-height"> \
	    		<div class="menu-panel full-height"> \
	    			<ul class="menu-list"> \
	    				<li id="pickerSearchTab" name="search" class="picker-tab search' + plugin.tabVisibilityClass('search') + '">' + plugin.settings.labels.tabSearch + '</li> \
	    				<li id="pickerBrowseTab" name="browse" class="picker-tab browse' + plugin.tabVisibilityClass('browse') + '">' + plugin.settings.labels.tabBrowse + '</li> \
	    				<li id="pickerUploadTab" name="upload" class="picker-tab upload' + plugin.tabVisibilityClass('upload') + '">' + plugin.settings.labels.tabUpload + '</li> \
	    			</ul> \
	    		</div> \
    	    	...
    	    	if (selectedTab.is('.picker-tab')) {
    	    		...
    	    		if (selectedPanel && selectedPanel.length) {
    	    			...
    	    			if(plugin.settings.panelVisibility[selectedTabName]) {
    	    				panels.find('.content-panel').addClass('hidden');
    	    				var newPanel = plugin.panel[selectedTabName](plugin.settings);
