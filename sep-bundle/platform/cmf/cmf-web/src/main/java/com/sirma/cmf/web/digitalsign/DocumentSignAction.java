...
public class DocumentSignAction extends EntityAction implements Serializable {
	...
	private Instance<InstanceDao<DocumentInstance>> documentInstanceDaoInstance;
	...
	public String reload(DocumentInstance instance) {
		...
		if (instance != null) {
			...
			log.error("Document: " + instance.getId() + " is not found in current case: "
					+ getDocumentContext().getInstance(CaseInstance.class));
		} else {
			log.error("Document provided is invalid for current case: "
					+ getDocumentContext().getInstance(CaseInstance.class));
		}
	...
	/**
	 * Checks if is sign allowed.
	 * 
	 * @param context
	 *            the context
	 * @return true, if is sign allowed
	 */
	public boolean isSignAllowed(Serializable context) {
		if (context instanceof DocumentInstance) {
			DocumentInstance doc = (DocumentInstance) context;
			String mimetype = getMimetype(doc);
			if ((mimetype != null) && (mimetype.endsWith("/pdf") || mimetype.endsWith("/xml"))) {
				if (doc.getProperties().get(DocumentProperties.DOCUMENT_SIGNED_DATE) != null) {
					return false;
				}
				return true;
			}
	...
	public String getMimetype(Serializable instance) {
		if (instance instanceof DocumentInstance) {
			...
			Serializable serializable = doc.getProperties().get(DocumentProperties.MIMETYPE);
