...
@Named
@ApplicationScoped
public class InstanceContextInitializer extends Action {
	...
	public void restoreHierarchy(Instance selectedInstance, Instance context) {
		...
		if (contextInstance == null) {
			...
			if ((owningInstance == null) || !(owningInstance instanceof InstanceContext)) {
				List<LinkReference> immediateParents = getImmediateParents(selectedInstance
						.toReference());
				...
				walkHierarchy(immediateParents.get(0).getTo().getIdentifier(), parentPath,
						immediateParents);
	...
	private boolean walkHierarchy(Serializable id, List<InstanceReference> parentPath,
			List<LinkReference> immediateParents) {
		// Get a parent instance. Check if is same as the context instance.
		for (LinkReference linkReference : immediateParents) {
		...
		for (LinkReference linkReference : immediateParents) {
			InstanceReference to = linkReference.getTo();
			List<LinkReference> linksTo = getImmediateParents(to);
			if (!linksTo.isEmpty()) {
				if (walkHierarchy(id, parentPath, linksTo)) {
					parentPath.add(to);
					return true;
				}
			}
	 ...
	 */
	private List<LinkReference> getImmediateParents(InstanceReference reference) {
		return linkService.getSimpleLinks(reference, LinkConstants.PART_OF_URI);
	}
}
