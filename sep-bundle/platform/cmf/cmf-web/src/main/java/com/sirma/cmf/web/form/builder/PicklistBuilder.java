...
public class PicklistBuilder extends ControlBuilder {
	...
	public UIComponent getComponentInstance() {
		...
		UICommandButton picklistTriggerButton = createPicklistTriggerButton(picklistPanelId,
				filterName, keywords);
	 ...
	 */
	private UICommandButton createPicklistTriggerButton(String picklistPanelId, String filterName,
		...
		String actionExpressionString = "#{picklistController.loadItems(false,'" + pickerItemsType
				+ "')}";
		MethodExpression actionExpression = createMethodExpression(actionExpressionString,
				new Class<?>[] {});

		// String disabledExpressionString = "";
		// ValueExpression disabledExpression = createValueExpression(disabledExpressionString,
		// Boolean.class);
		ValueExpression disabledExpression = null;

		Map<String, ControlParam> controlParameters = getAsMap(propertyDefinition
				.getControlDefinition().getControlParams());
		String oncompleteAttribute = createOncompleteAttribute(controlParameters, btnSecificClass);
		...
		String render = picklistPanelId;
		String execute = "@this";
		String styleClass = BuilderCssConstants.CMF_STANDARD_BUTTON + " open-picklist "
				+ btnSecificClass;
		Boolean rendered = Boolean.TRUE;
		Boolean immediate = Boolean.TRUE;

		UICommandButton picklistTriggerButton = builderHelper.createAjaxButton(id,
				actionExpression, disabledExpression, oncompleteAttribute, triggerButtonValue,
				render, execute, styleClass, rendered, immediate);

		picklistTriggerButton.getAttributes().put(PicklistConstants.FILTERNAME_ATTR, filterName);
		if (!keywords.isEmpty()) {
			picklistTriggerButton.getAttributes().put(PicklistConstants.KEYWORDS_ATTR, keywords);
		}

		picklistTriggerButton.setValueExpression("data",
				createValueExpression("#{requestScope.picklistItems}", Object.class));

		return picklistTriggerButton;
	}

	/**
	 * Creates the oncomplete attribute for the picklist trigger button.
	 * 
	 * @param parameters
	 *            the parameters
	 * @param btnSecificClass
	 *            the btn secific class
	 * @return oncomplete attribute as string
	 */
	private String createOncompleteAttribute(Map<String, ControlParam> parameters,
			String btnSecificClass) {
		StringBuilder result = new StringBuilder();
		result.append("jQuery('." + btnSecificClass
				+ "').trigger({type : 'openPicklist', itemsList : event.data");
		result.append("});");
	...
	private UIComponent createPreviewField(boolean previewMode) {
		...
		previewField.setId("picklistPreviewField-" + propertyDefinition.getIdentifier());
