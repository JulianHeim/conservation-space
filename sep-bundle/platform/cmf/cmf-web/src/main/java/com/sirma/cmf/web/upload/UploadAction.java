...
public class UploadAction extends NewDocumentAction implements Serializable {
	...
	public void listener(FileUploadEvent event) throws Exception {
		...
		// CMF-765: if we have common document then the title should be null
		// if (EqualsHelper.nullSafeEquals(docTitle,
		// CommonProperties.DEFAULT_DOCUMENT_ATTACHMENT_TYPE, false)) {
		// setTitle(null);
		// }
		...
		try {
			...
			eventService.fire(new FileUploadedEvent(selectedDocumentInstance, getDocumentContext()
					.getCurrentInstance()));
			...
			if (tempDir != null) {
				...
				documentService.upload(sectionForSelectedDocument, true, selectedDocumentInstance);
				// instanceService.attach(sectionForSelectedDocument, new Operation(
				// ActionTypeConstants.UPLOAD), selectedDocumentInstance);
	...
	private File updateDocumentModel(org.richfaces.model.UploadedFile item,
		...
		uploadedFile.setData(item.getData());
	...
	static File updateDocumentModelOnUpload(UploadedFile uploadedFile,
		...
		long size = IOUtils.copyLarge(new ByteArrayInputStream(uploadedFile.getData()), output);
		...
		properties.put(DocumentProperties.MIMETYPE, uploadedFile.getMimetype());
