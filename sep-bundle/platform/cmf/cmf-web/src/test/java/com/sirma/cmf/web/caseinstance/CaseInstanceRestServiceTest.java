...
public class CaseInstanceRestServiceTest extends CMFTest {
	...
	/** The service. */
	private CaseInstanceRestService service;
	...
	public CaseInstanceRestServiceTest() {
		...
		ReflectionUtils.setField(service, "caseService", caseService);
	...
	public void getObjectSectionsTest() {
		...
		Response response = service.getObjectSections(null, null, null);
		assertEquals(response.getStatus(), Response.Status.BAD_REQUEST.getStatusCode());

		//
		response = service.getObjectSections("caseid", null, null);
		assertEquals(response.getStatus(), Response.Status.BAD_REQUEST.getStatusCode());
		//
		response = service.getObjectSections(null, "sectioninstance", null);
		assertEquals(response.getStatus(), Response.Status.BAD_REQUEST.getStatusCode());

		// if section can't be found
		Mockito.when(sectionService.loadByDbId(Mockito.any(Serializable.class))).thenReturn(null);
		response = service.getObjectSections("sectionid", "sectioninstance", null);
