...
public class CaseActionTest extends CMFTest {
	...
	public void deleteCaseTest() {
		...
		// check if service is actually invoked once
		Mockito.verify(caseService, Mockito.times(1)).delete(caseInstance, operation, false);
		// check if default case search is executed once
		Mockito.verify(caseSearchAction, Mockito.times(1)).executeDefaultCaseSearch();
		// check if context is cleared
		assertTrue(caseAction.getDocumentContext().isEmpty());
		// test navigation
		assertEquals(NavigationConstants.NAVIGATE_CASE_LIST_PAGE, event.getNavigation());
	}
