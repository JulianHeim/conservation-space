...
public class RelationsRestServiceTest extends CMFTest {
		...
		controller = new RelationsRestService() {
			...
			protected JSONArray buildData(InstanceReference instance) {
				if (callSuperBuildData) {
					return super.buildData(instance);
				}
				return responseData;
			}
	...
	public void loadDataTest() {
		...
		Response expectedResponse = controller.loadData(null, null, null, null, null);
		...
		// if we don't pass all required arguments
		expectedResponse = controller.loadData("1", null, null, null, null);
		assertEquals(expectedResponse.getStatus(), Response.Status.BAD_REQUEST.getStatusCode());
		assertNotNull(expectedResponse.getEntity());

		// if we don't pass all required arguments
		expectedResponse = controller.loadData(null, "caseinstance", null, null, null);
		assertEquals(expectedResponse.getStatus(), Response.Status.BAD_REQUEST.getStatusCode());
		assertNotNull(expectedResponse.getEntity());
		...
		// response
		caseInstance = createCaseInstance(Long.valueOf(1));
		expectedResponse = controller.loadData("1", "caseinstance", null, null, null);
	...
	public void buildDataTest() {
		...
		JSONArray data = controller.buildData(null);
	...
	public void deactivateRelationTest() {
		Response expectedResponse = controller.deactivateRelation(null);
		...
		expectedResponse = controller.deactivateRelation("1");
		assertEquals(expectedResponse.getStatus(), Response.Status.OK.getStatusCode());
	}
