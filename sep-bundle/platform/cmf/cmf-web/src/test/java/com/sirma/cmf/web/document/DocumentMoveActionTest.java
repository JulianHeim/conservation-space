...
public class DocumentMoveActionTest extends CMFTest {
	...
	/**
	 * Accept selected target test.
	 */
	public void acceptSelectedTargetTest() {
		String navigation = action.acceptSelectedTarget();
		assertNull(navigation);
		assertNull(action.getSelectedSectionInstance());
		assertNull(action.getSelectedDocumentForMove());

		//
		Action moveToOtherCaseAction = new EmfAction(ActionTypeConstants.MOVE_OTHER_CASE);
		action.setAction(moveToOtherCaseAction);
		action.acceptSelectedTarget();
		assertNull(action.getSelectedSectionInstance());
		assertNull(action.getSelectedDocumentForMove());
		assertTrue(invokedMoveToOtherCase);
	}
