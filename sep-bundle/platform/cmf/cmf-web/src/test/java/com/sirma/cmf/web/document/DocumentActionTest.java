...
public class DocumentActionTest extends CMFTest {
	...
	public void deleteTest() {
		...
		// if we don't have instance in context we expect navigation to case documents page
		assertEquals(event.getNavigation(), NavigationConstants.RELOAD_PAGE);

		// if we have instance in context, we expect the page to be reloaded
		action.getDocumentContext().setDocumentInstance(documentInstance);
		action.delete(event);
	...
	public void getHistoricalDocVersionTest() {
		...
		Mockito.when(documentService.getDocumentVersion(docInstance, version))
