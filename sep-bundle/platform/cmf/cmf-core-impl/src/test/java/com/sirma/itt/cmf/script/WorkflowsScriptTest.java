...
public class WorkflowsScriptTest extends CmfTest {
	...
	public void testDocumentStateChange() {
		...
		ReflectionUtils.setField(context, "reference", new LinkSourceId("workflow", new DataType(),
				context));
		...
		ReflectionUtils.setField(document, "reference", new LinkSourceId("document",
				new DataType(), document));

		List<LinkReference> references = new ArrayList<LinkReference>(1);
		LinkReference ref = new LinkReference();
		ref.setFrom(new LinkSourceId("workflow", new DataType()));
		ref.setTo(new LinkSourceId("document", new DataType()));
		references.add(ref);
		Mockito.when(
				linkService.getLinks(Mockito.any(InstanceReference.class),
						Mockito.eq("emf:processes")))
