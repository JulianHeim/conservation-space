...
import com.sirma.itt.emf.search.SearchEventBinding;
import com.sirma.itt.emf.search.SearchEventObject;
import com.sirma.itt.emf.search.SearchEventType;
...
public class CaseDocumentDmsSearchEngineExtension implements DmsSearchEngineExtension {
	...
	public <E extends CaseInstance> SearchArguments<E> searchCases(SearchArguments<E> arguments) {
		...
		SearchArguments<DMSFileDescriptor> searchArguments = new SearchArguments<DMSFileDescriptor>();
	 ...
	 * @return the list
	 */
	private <E extends DMSFileDescriptor> List<CaseInstance> performSearch(SearchArguments<E> args,
			String searchType) {
		...
		for (CaseInstance caseInstance : result) {
			...
			for (InstanceReference reference : new LinkReferenceIterable(links)) {
		...
		for (Entry<InstanceReference, CaseInstance> entry : caseMapping.entrySet()) {
			...
			for (InstanceReference reference : new LinkReferenceIterable(sections)) {
	...
	public <C extends CaseInstance, D extends List<DocumentInstance>> SearchArgumentsMap<C, D> searchCasesAndDocuments(
		...
		SearchArgumentsMap<DMSFileDescriptor, List<DMSFileDescriptor>> searchArguments = new SearchArgumentsMap<DMSFileDescriptor, List<DMSFileDescriptor>>();
		Query query;
		if (arguments.getQuery() == null) {
			Map<String, Serializable> map = arguments.getArguments();
	 ...
	 */
	@SuppressWarnings("unchecked")
	<E extends DMSFileDescriptor, D extends DMSFileDescriptor> Map<CaseInstance, List<DocumentInstance>> performMapSearch(
	 ...
	 * @return the list
	 */
	private <E extends DMSFileDescriptor> Map<CaseInstance, List<DocumentInstance>> performDocumentMapSearch(
