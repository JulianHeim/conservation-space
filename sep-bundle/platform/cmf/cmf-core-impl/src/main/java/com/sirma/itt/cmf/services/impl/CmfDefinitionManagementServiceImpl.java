...
public class CmfDefinitionManagementServiceImpl implements DefinitionManagementServiceExtension {
	 ...
	 */
	private List<DMSFileDescriptor> getDefinitionInternal(Class<?> definitionClass) {
		try {
	 ...
	 */
	private List<DMSFileDescriptor> getTypeDefinitions() {
		List<DMSFileDescriptor> result = new ArrayList<DMSFileDescriptor>(1);
		// add the specific types
		result.add(new ResourceFileDescriptor("../types.xml", CmfSchemaBuilder.class));
