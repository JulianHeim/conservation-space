...
import com.sirma.itt.emf.util.LinkReferenceIterable;
...
public class SectionInstanceDao extends
		BaseInstanceDaoImpl<SectionInstance, SectionEntity, String, String, SectionDefinition> {
	...
	protected Iterable<InstanceReference> getCurrentChildren(SectionInstance targetInstance) {
		...
		return new LinkReferenceIterable(list);
	}

	@Override
	protected void attachNewChildren(SectionInstance targetInstance, Operation operation,
	...
	@Override
	protected void detachExistingChildren(SectionInstance sourceInstance, Operation operation, List<Instance> oldChildren) {
	 ...
	 */
	@Override
	protected void onAfterSave(SectionInstance instance) {
	...
	public SectionInstance persistChanges(SectionInstance instance) {
	 ...
	 */
	@Override
	public void synchRevisions(SectionInstance instance, Long revision) {
	 ...
	 */
	@Override
	protected void populateInstanceForModel(SectionInstance instance, SectionDefinition model) {
	...
	protected <E extends Instance> void loadChildren(E instance, boolean forBatchLoad,
		...
		if (instance instanceof SectionInstance) {
			...
			List<Instance> children = BatchEntityLoader.loadFromReferences(
					new LinkReferenceIterable(list), serviceRegister, taskExecutor);
	...
	// public void loadProperties(SectionInstance instance) {
	// propertiesService.loadProperties(instance);
	// }
	//
	// @Override
	// protected <E extends Instance> void batchFetchProperties(List<E> toLoadProps, boolean
	// loadAll) {
	...
	class InstancePersistTask extends GenericAsyncTask {
		...
		protected boolean executeTask() {
			...
			try {
				...
				if ((operationId != null) && (operationId instanceof String)) {
		...
		public void executeOnSuccess() {
			...
			if (eventProvider != null) {
				InstancePersistedEvent<Instance> persistedEvent = eventProvider
						.createPersistedEvent(getInstance(), oldCopy);
	...
	class SectionEntityLookupDao extends DefaultEntityLookupDao {
		...
		protected List<SectionEntity> fetchEntityByValue(String key) {
			return getDbDao().fetchWithNamed(DbQueryTemplates.QUERY_SECTION_BY_DMS_ID_KEY,
