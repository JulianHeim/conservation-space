...
public class WorkflowServiceImpl implements WorkflowService {
	...
	public List<TaskInstance> startWorkflow(WorkflowInstanceContext context,
		...
		try {
			...
			if (startWorkflowTrans == null) {
				throw new CmfDefinitionValidationException("The workflow's "
			...
			TaskInstance startTask = updateStartTask(context, definition, startTaskDef,
					startTaskInstance);
			...
			// startTask.setWorkflowInstanceId(context.getWorkflowInstanceId());
			...
			List<TaskInstance> startTaskList = new ArrayList<TaskInstance>(1);
			startTaskList.add(startTask);
			taskService.attachTaskToInstance(owningInstance, startTaskList, context, false);
	...
	private TaskInstance updateStartTask(WorkflowInstanceContext context,
			WorkflowDefinition definition, TaskDefinitionRef startTaskDef, TaskInstance instance) {
	...
	public List<TaskInstance> updateWorkflow(WorkflowInstanceContext context,
		...
		try {
			...
			// taskService.save(taskInstance, new Operation(OperationType.SAVE));
	...
	public WorkflowInstanceContext save(WorkflowInstanceContext instance, Operation operation) {
		...
		try {
			...
			eventService.fire(new WorkflowPersistedEvent(instance, old));
		} finally {
			if (operation != null) {
				RuntimeConfiguration
						.clearConfiguration(RuntimeConfigurationProperties.CURRENT_OPERATION);
