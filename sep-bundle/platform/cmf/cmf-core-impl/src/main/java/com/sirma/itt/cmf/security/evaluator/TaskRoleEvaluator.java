...
public class TaskRoleEvaluator extends BaseRoleEvaluator<TaskInstance> implements
	...
	@Inject
	@RoleEvaluatorType(value = ObjectTypesCmf.WORKFLOW_TASK, scope = EvaluatorScope.EXTERNAL)
	private Instance<RoleEvaluator<TaskInstance>> externalEvaluator;
	...
	protected Pair<Role, RoleEvaluator<TaskInstance>> evaluateInternal(TaskInstance target,
		...
		// if he owns the task
		if (resourceService.equals(resource, properties.get(TaskProperties.TASK_OWNER))) {
		...
		} else if (resourceService.equals(resource,
				parent.getProperties().get(WorkflowProperties.CREATED_BY))
				|| resourceService.equals(resource,
