...
public class DocumentExtractEvaluator extends BaseEvaluator {
	...
	protected Serializable evaluateInternal(Matcher matcher, ExpressionContext context,
		...
		if (documents.isEmpty()) {
			...
			// if more then one sort them by modification
			Collections.sort(documents, new PropertyModelComparator(false,
					DocumentProperties.MODIFIED_ON));
		}

		DocumentInstance instance = documents.get(0);
		return instance.getProperties().get(fieldName);
	}
