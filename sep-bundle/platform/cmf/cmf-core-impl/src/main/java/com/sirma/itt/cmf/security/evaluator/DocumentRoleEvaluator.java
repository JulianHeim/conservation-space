...
public class DocumentRoleEvaluator extends BaseRoleEvaluator<DocumentInstance> implements
	...
	/** The upload. */
	static final Action UPLOAD = new EmfAction("document_upload");
	/** The Constant PRINT. */
	...
	/** The no document action action. */
	private Set<Action> noDocumentActionActions;
	...
	public void initialize() {
		...
		noDocumentActionActions = CollectionUtils.createLinkedHashSet(2);
		noDocumentActionActions.add(UPLOAD);
	}
	...
	protected Pair<Role, RoleEvaluator<DocumentInstance>> evaluateInternal(DocumentInstance target,
		...
		if (resource == null) {
			LOGGER.warn("Not a valid null resource ");
			return null;
		}
		if (resourceService.equals(resource,
		...
		if (primaryContainer == null) {
			LOGGER.warn("No parent found for document: {}", target != null ? target.getIdentifier()
					: " null");
			// TODO: add additional checks for documents from document library
			return constructRoleModel(primaryContainer, resource, BaseRoles.CONSUMER,
	...
	protected Boolean filterInternal(DocumentInstance target, Resource resource, Role role,
		...
		if (isInstanceInStates(primaryContainer, PrimaryStates.CANCELED)) {
			...
			return Boolean.TRUE;// bbanchev optimization
		}
		...
		boolean creator = resourceService.equals(resource,
		...
		if (target.getProperties().containsKey(DocumentProperties.DOCUMENT_SIGNED_DATE)) {
			actions.remove(DOCUMENT_SIGN);
		}
		...
		if (role.getPermissions().containsKey(SecurityModel.PERMISSION_LOCK)) {
			...
			if (lockedBy == null) {
				...
				// operations
				if (!resourceService.equals(resource, lockedBy)) {
					// only administrators can unlock
					if (!role.getRoleId().equals(ADMINISTRATOR)) {
						actions.remove(UNLOCK);
						actions.remove(CANCEL_EDIT_OFFLINE);
						actions.remove(UPLOAD_NEW_VERSION);
						actions.remove(DOCUMENT_SIGN);
