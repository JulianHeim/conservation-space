...
public class DueDateCalculator {
	...
	public void calculateDueDateForEgov(@Observes WorkflowCreateEvent event) {
		...
		setField(instance, "serviceTermType", context, WorkflowProperties.PRIORITY, false);
		setField(instance, "serviceDueDate", context, WorkflowProperties.PLANNED_END_DATE, true);
		if (context.getProperties().get(WorkflowProperties.PLANNED_END_DATE) == null) {
			// if the due date is not passed via the service we should fill it
			// with something
			calculateDueDateAndPriorityInternal(context);
	 ...
	 *            the is date
	 */
	private void setField(Instance caseInstance, String caseKey, Instance context,
			String contextKey, boolean isDate) {
		Serializable value = caseInstance.getProperties().get(caseKey);
		if (value == null) {
			return;
		}
		if (isDate) {
			if (value instanceof Date) {
				context.getProperties().put(contextKey, value);
			} else {
				LOGGER.warn("The fill {} was not a java.util.Date but was {} with value {}",
						contextKey, value.getClass(), value);
				return;
			}
		} else {
			context.getProperties().put(contextKey, value);
	...
	private void calculateDueDateAndPriorityInternal(Instance context) {
		...
		} else if (serializable == null) { // the check is not necessary but
											// just in case
