...
public class DocumentInstanceDao extends
	...
	protected void onInstanceUpdated(DocumentInstance instance) {
		...
		if (properties.get(DocumentProperties.CREATED_BY) == null) {
			setCurrentUserTo(instance, DocumentProperties.CREATED_BY);
		}
		Date date = new Date();
		if (properties.get(DocumentProperties.CREATED_ON) == null) {
			properties.put(DocumentProperties.CREATED_ON, date);
		}

		setCurrentUserTo(instance, DocumentProperties.MODIFIED_BY);
		properties.put(DocumentProperties.MODIFIED_ON, date);
		// set initial version
	...
	protected <E extends Entity<String>> List<E> findEntities(Set<String> ids) {
		List<E> list = getDbDao().fetchWithNamed(DbQueryTemplates.QUERY_DOCUMENT_ENTITIES_BY_DMS_KEY,
	...
	protected <E extends Entity<String>> List<E> findEntitiesByPrimaryKey(Set<String> ids) {
		List<E> list = getDbDao().fetchWithNamed(DbQueryTemplates.QUERY_DOCUMENT_ENTITIES_BY_ID_KEY,
	...
	public class DocumentEntityLookup extends DefaultEntityLookupDao {
		...
		protected List<DocumentEntity> fetchEntityByValue(String key) {
			...
			return getDbDao().fetchWithNamed(DbQueryTemplates.QUERY_DOCUMENT_ENTITY_BY_DMS_KEY, args);
		}
	}
