...
import com.sirma.itt.cmf.event.task.standalone.StandaloneTaskChangeSaveEvent;
...
public class StandaloneTaskServiceImpl implements StandaloneTaskService {
	...
	public StandaloneTaskInstance createInstance(TaskDefinition definition, Instance parent,
		...
		if (context instanceof DMSInstance) {
			instance.setParentContextId(((DMSInstance) context).getDmsId());
		}
	...
	public StandaloneTaskInstance start(StandaloneTaskInstance instance, Operation operation) {
		...
		try {
			...
			// if (transition == null) {
			// throw new CmfRuntimeException("No transition definition found for operation="
			// + startTransition + " id task definition " + instance.getDefinitionId());
			// }
			...
			eventService.fire(new StandaloneTaskChangeSaveEvent(instance));

			// instance.getProperties().remove(TaskProperties.TASK_ASSIGNEE);
			...
			// !taskService.get().isPooledTask(instance)
					);
	...
	public StandaloneTaskInstance complete(StandaloneTaskInstance instance, Operation operation) {
		...
		try {
			...
			if (operation != null) {
				transition = operation.getOperation();
			}
