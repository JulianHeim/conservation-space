...
public class AlfrescoUploader implements AlfrescoCommunicationConstants {
	 ...
	 */
	public String uploadFile(String baseServiceURI, DMSFileDescriptor descriptor, String siteId,
	 ...
	 *             on any dms/connection error on any dms/connection error or malformed request
	 */
	public String uploadFile(DMSFileDescriptor descriptor, String siteId, String folder,
	 ...
	 *             on any dms/connection error on any dms/connection error or malformed request
	 */
	public String updateFile(String updateService, DMSFileDescriptor descriptor, String dmsId,
	...
	protected String upload(String uri, Part[] parts) throws DMSClientException {
		HttpMethod method = restClient.createMethod(new PostMethod(), parts, true);
		String result = restClient.request(uri, method);
		return result;
	}
	 ...
	 * @return the descriptor or throws exception on error
	 */
	public PartSource getPartSource(DMSFileDescriptor descriptorInput) {

		DMSFileDescriptor descriptor = descriptorInput;
		if (descriptor instanceof UploadWrapperDescriptor) {
			// downgrade it
			descriptor = ((UploadWrapperDescriptor) descriptor).getDelegate();
		}
		if (descriptor == null) {
			throw new DmsRuntimeException("Upload data is not provided!");
		}
		try {
			if (descriptor instanceof LocalProxyFileDescriptor) {
				...
				return new ByteArrayPartSource(descriptor.getId(), IOUtils.toByteArray(descriptor
						.download()));
			}
			return new ByteArrayPartSource(descriptor.getId(), IOUtils.toByteArray(descriptor
					.download()));
